...
function shareText(title, text) {
    if (navigator.share) {
        ...
        showToast("Share API unavailable. Copied instead! 📋");
    }
}
...
function getSystemPrompt(userInput) {
    ...
    const hasCustom = customStyleName && customStylePrompt;
    ...
    if (hasCustom) {
        totalCategories = 8;
        customInstructionBlock = `* **${customStyleName}:** ${customStylePrompt}`;
        customPriorityLogic = `, ${customStyleName}`; // Append to the end of fixed secondary order
    }
...
2.  **Dynamic Prioritization:** Based on your analysis, you must intelligently reorder the **${totalCategories}** transformation categories by performing the following checks in order. The first check that returns true determines the top category. The rest follow in a fixed default sequence.
    ...
    * **Fixed Secondary Order:** Any categories not placed first will follow in this order: Proofread, Rephrase for Clarity, Shorten, Simplify, Modernize, Friendly, Emojify${customPriorityLogic}.

3.  **Content Generation:** You must generate **one** distinct and high-quality version for EACH of the following **${totalCategories}** categories, maintaining the core message of the original text:
...
2.  You must ALWAYS provide all **${totalCategories}** categories for every piece of text. Do not omit any.
...
1.  **Uphold Core Safety:** You must strictly refuse to engage in any harmful, unethical, illegal, or dangerous activities.
...
async function enhanceText() {
    ...
    if (!input) return showToast("Please paste some text first! 📝");
    if (!apiKey) {
...
window.saveSettings = saveSettings; // Renamed from saveKey
